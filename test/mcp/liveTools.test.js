...
test("agent can create a paper session, feed price, place a bracket, and flatten", async () => {
  ...
  await liveTools.feed_price.handler({ sessionId: "demo", bar: { time: 1, open: 100, high: 100, low: 100, close: 100, volume: 1 } });
  const order = await liveTools.place_order.handler({ sessionId: "demo", side: "long", type: "market", riskPct: 1, stop: 98, target: 104 });
  ...
  await liveTools.feed_price.handler({ sessionId: "demo", bar: { time: 2, open: 104, high: 104, low: 104, close: 104, volume: 1 } });
...
test("place_order on an unknown session errors clearly", async () => {
  await assert.rejects(() => liveTools.place_order.handler({ sessionId: "ghost", side: "long", type: "market", qty: 1 }));
});

test("live mode is refused without gating", async () => {
  await assert.rejects(() => liveTools.create_session.handler({ sessionId: "l1", symbol: "AAPL", mode: "live", confirmLive: true }));
});
